...
class DeclarationsVisitor(var scope: ScopeManager) : CPP14BaseVisitor<MutableList<Declaration>>() {
  ...
  override fun visitParametersandqualifiers(ctx: CPP14Parser.ParametersandqualifiersContext): MutableList<Declaration> {
    var list = ctx.parameterdeclarationclause().accept(this)

    return list
  }


  override fun visitParameterdeclarationlist(ctx: CPP14Parser.ParameterdeclarationlistContext?): MutableList<Declaration> {
    val list = mutableListOf<Declaration>()

    ctx?.parameterdeclarationlist()?.accept(this)?.let { list.addAll(it) }
    ctx?.parameterdeclaration()?.accept(this)?.let { list.addAll(it) }

    return list
  }

  override fun visitParameterdeclaration(ctx: CPP14Parser.ParameterdeclarationContext): MutableList<Declaration> {
    val list = mutableListOf<Declaration>()
    ...
    val declaration = ctx.accept(ValueDeclarationVisitor(scope, ParamVariableDeclaration::class.java, type))
...
class DeclarationVisitor(
  ...
  override fun visitNoptrdeclarator(ctx: CPP14Parser.NoptrdeclaratorContext): NamedDeclaration {
    ...
    if (ctx.parametersandqualifiers() != null && ctx.noptrdeclarator() != null) {
      ...
      // parameters
      val parameters = mutableListOf<ParamVariableDeclaration>()

      // just to make sure, filter out only param variable declarations
      ctx.parametersandqualifiers().accept(DeclarationsVisitor(scope))
          .filterIsInstance<ParamVariableDeclaration>()
          .forEach { parameters.add(it) }
